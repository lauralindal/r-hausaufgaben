...
c) Berechnen Sie folgende bedingte relative Häufigkeiten in Prozent:
  ...
  Unter den Überlebenden waren `r i3` % Frauen.
  Unter den Überlebenden waren `r i4` % Männer.
...
```{r}
mosaicplot(Sex ~ Survived2, data = titanic_data, main = "Überlebende auf der Titanic - Mosaikplot", 
           ylab="Überlebt?", xlab="Geschlecht", col = c("turquoise", "palegreen1"))
```
...
mosaicplot(tab_1, xlab = "Gender", ylab = "Survived?", main = "Survivors in First Class", col = c("pink", "hotpink3"))
mosaicplot(tab_2, xlab = "Gender", ylab = "Survived?", main = "Survivors in Second Class", col = c("lightgreen", "green3"))
mosaicplot(tab_3, xlab = "Gender", ylab = "Survived?", main = "Survivors in Third Class", col = c("mediumpurple1", "mediumorchid4"))
```
...
In der zweiten Klasse war das Geschlecht der Reisenden am Relevantesten für deren Überleben (im Vergleich zu den anderen Klassen). Es besteht hier nämlich ein starker Zusammenhang (`r p2`) zwischen Geschlecht und Überleben. Bei der ersten Klasse ist der Zusammenhang zwischen Geschlecht und Überleben noch mäßig relevant (`r p1`). In der dritten Klasse besteht zwischen den beiden Merkmalen nur noch ein schwacher Zusammenhang (`r p3`).
...
a) Gruppieren Sie `Age` wie in Aufgabe 3 der ersten Hausarbeit in die Intervalle [0,5), [5,10), [10, 15) usw! Beachten Sie wieder, zu welcher Gruppe die Grenzwerte der Intervalle gehören. Hinweis: Sehen Sie sich die Funktion `cut()` an, deren Syntax der von `hist()` ähnelt.
    ...
    # Gruppierung von Age
    bins <- c(seq(0,85,5))
    age_group <- table(cut(titanic_data$Age, breaks = bins, include.lowest = TRUE, right = FALSE))
    age_group
...
h_age_survival <- table(titanic_data$AgeGroup, titanic_data$Survived2) # leider geht hier [75, 80) verloren (weil 0? oder wegen den Levels?)
...
# Ermittle bedingte relative Häufigkeiten f( survived | $age_group )
h_survivors <- matrix(h_age_survival[,2], nrow = 17, ncol = 1,  dimnames=list(c("[0,5)", "[5,10)", "[10,15)", "[15,20)", "[20,25)", "[25,30)", "[30,35)", "[35,40)", "[40,45)", "[45,50)", "[50,55)", "[55,60)", "[60,65)", "[65,70)", "[70,75)", "[75,80)", "[80,85)"), c("Survived")))
f_survivors <- prop.table(h_survivors)
f_survivors_rounded <- round(f_survivors * 100, 2)
kable(f_survivors_rounded)

# Ermittle Überlebenschance (relative Häufigkeit über Zeile)

f_age_survival_margins_1 <- f_age_survival_margins
for(i in 1:17){
  f_age_survival_margins_1[i,2] <- (f_age_survival_margins[i,2] / f_age_survival_margins[i,3]) * 100
}

f_age_survival_margins_1 <- f_age_survival_margins_1[,2]
f_age_survival_margins_1 <- f_age_survival_margins_1[-18]

foo <- data.frame(f_age_survival_margins_1)
kable(foo)

# Überlebenschance einzelne Zeile
prop.table(h_age_survival[3,])
```
...
```{r}
dims <- c("[0,5)", "[5,10)", "[10,15)", "[15,20)", "[20,25)", "[25,30)", "[30,35)", "[35,40)", "[40,45)", "[45,50)", "[50,55)", "[55,60)", "[60,65)", "[65,70)", "[70,75)", "[80,85)")

foo$f_age_survival_margins_1

barplot(foo$f_age_survival_margins_1[1:16], names.arg = dims, main = "Überlebenschance: f( survived | $age_group )", xlab = "Age Group", ylab = "Relative Häufigkeit", cex.names = 0.8, col = "orchid3")
```
